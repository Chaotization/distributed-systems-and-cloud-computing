...
public class Dht extends DhtBase implements IDhtService, IDhtNode, IDhtBackground {
	...
	// WebMethod
	public OptNodeBindings notify(NodeBindings predDb) throws Failed {
		/*
		...
		if (!pred .hasNodeInfo() || inInterval(cand.getId(), pred.getNodeInfo().getId(), info.getId(), false)) {
			...
			synchronized (state) {
				...
				for (Bindings binding : db.getBindingsList()) {
					EventBroadcaster.getInstance().broadcastMovedBinding(binding.getKey());
				}
	...
	// Webmethod
	@Override
	public void listenOn(int listenerId, String key, EventProducer eventProducer) throws Failed {
		...
		if (routing.getPred() == null ||
			...
			eventProducer.onNewBinding(key, Integer.toString(id));
	...
	public void stopListening(String key) throws DhtBase.Failed {
		 ...
		 */
		Subscription subscription = Subscription.newBuilder().setId(getNodeInfo().getId()).setKey(key).build();
		NodeInfo target = findSuccessor(NodeKey(key));
